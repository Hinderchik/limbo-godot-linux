<table>
  <tr>
    ...
    <td style="width:50%">
      <h3>A window-based, LIMBO-inspired, key-shuffle minigame made with Godot Engine, for Linux</h3>
    </td>
  </tr>
</table>
...
- You can adjust several toggles on the setting menu, some of the useful one are:
  ...
  - `no_ending_screen`, when turned on, the terminal sequence won't happen, instead after clicking on the key, it'll just say if the key you choose is correct or wrong. For quick play.
  - `fullscreen_ending`, when turned off, the ending sequence will play on maximized borderless window.
  - The rest are useful for debugging, and information about each variable can be read on the main root node's script.
- Pressing `Esc` during the terminal ending will make you instantly quit the game, instead of needing to wait for 5 seconds
...
- The save file is located in `~/.local/share/limbo-godot/limbosave.cfg`, while the log file is on `~/.local/share/limbo-godot/logs/limbo.log`.
...
- [Selawik](https://github.com/microsoft/Selawik) by Microsoft
...
- Godot Engine by Juan Linietsky, Ariel Manzur & Contributors

### Special Thanks
- quasar098's [limbos32](https://github.com/quasar098/limbos32) as main inspiration
- MarkHermy3100's [LimboKeys](https://github.com/MarkHermy3100/LimboKeys) for key shuffler pattern
- enderprism for sprite rip from [godot-dash-v2](https://github.com/enderprism/godot-dash-v2)
- Godot Official Discord Server
